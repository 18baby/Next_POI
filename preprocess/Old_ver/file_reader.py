...
import logging
import ast
from Next_POI_data_Preprocess.preprocess.Original.preprocess_fn import id_encode, ignore_first, only_keep_last
...
class FileReader(FileReaderBase):
    ...
    def generate_traj_id(cls, df, session_time_interval=24, rv_thresh=2):
        ...
        input: 
            df: 데이터프레임
            session_time_interval: 세션 시간 간격
            rv_thresh: 최초 유효 체크인 개수
            only_last_metric: 마지막 메트릭 여부
        output:
            df: 데이터프레임
        """
        df = df.sort_values(by=['SplitTag', 'UserId', 'LocalTime'], ascending=True).copy()

        # 고정 24시간 창 기준 trajectory 생성
        time_window = pd.Timedelta(hours=session_time_interval)
        
        traj_idxs = []
        
        for user_id, user_df in df.groupby('UserId'):
            user_df = user_df.sort_values(by='LocalTime')   # 사용자 체크인 시간순 정렬
            session_start = user_df['LocalTime'].iloc[0]    # 세션 시작 시간
            session_end = session_start + time_window       # 세션 종료 시간
            
            traj_idx = 1
            for idx, row in user_df.iterrows():
                if row['LocalTime'] <= session_end:
                    traj_idxs.append(f"{user_id}_{traj_idx}")
                else:
                    traj_idx += 1
                    session_start = row['LocalTime']
                    session_end = session_start + time_window
                    traj_idxs.append(f"{user_id}_{traj_idx}")
        df['TrajectoryId'] = traj_idxs

        logging.info(f'[Preprocess] make TrajectoryId done.')

        # check-in이 너무 적은 trajectory 제거
        counts = df['TrajectoryId'].value_counts()
        keep_counts = counts[counts >= rv_thresh].index
        df['keep_trajectory'] = False
        df.loc[df['TrajectoryId'].isin(keep_counts), 'keep_trajectory'] = True

        logging.info(f'[Preprocess] dropped trajectories len<{rv_thresh}: {len(df) - len(df[df["keep_trajectory"]])}]')

        return df[df["keep_trajectory"]]